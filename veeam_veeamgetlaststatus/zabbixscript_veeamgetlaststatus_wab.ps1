...
$zabbixserver ="192.168.0.45"
#zabbix name of host
$zabbixhostname = "CSW-MS02" 
...
#Get backup results
$JobId = Get-VBREPJob -Name $VeeamJobname | Select -ExpandProperty Id
$Job = Get-VBREPSession | Where-Object { $_.JobId -eq $JobId } | Sort -Descending -Property "CreationTime" | Select -First 1
$Job_Name = Get-VBREPJob -Name $VeeamJobname

#Get job and his last result.
if($Job_Name.Name -eq $VeeamJobname){
    fnclog "Getting job result - OK - job found: $VeeamJobname"
}else{
    fnclog "Getting job result - Error - job $VeeamJobname not found"
    Disconnect-VBRServer
	fnczabbixsend -zabbixhostname "$zabbixhostname" -zabbixitemkey "$zabbixitemkey" -zabbixitemkeyvalue "10"
    fncscripterrorsend;
}
$backupresultlast = $Job.Result
$backupresultlastCreationtime =$Job.CreationTime
$backupresultlastEndtime = $Job.EndTime
fnclog "Getting job result - backupresultlast: $backupresultlast"
fnclog "Getting job result - backupresultlastCreationtime: $backupresultlastCreationtime"
fnclog "Getting job result - backupresultlastEndtime: $backupresultlastEndtime"
#$backupLatestRunLocal = $Job.LatestRunLocal
#$backupresultlast = $Job.LastResult
#$backupresultlast = $Job.LastResult


fnclog "Getting job result - backupLatestRunLocal $backupresultlast"

#$backupresultlast2 = $Job.FindLastSession().Result
#fnclog "Getting job result - backupresultlast $backupresultlast2"
